...
class DeployTest(DeployBaseTest):
    ...
    @patch.object(Build, "copytree")
    @patch.object(Build, "create_artefact")
    def test_run_with_version(self, create_artefact_mock, copytree_mock, pip_install_mock):
        zip_file = MockZipFile.create_zip("test")
        create_artefact_mock.return_value = zip_file

        self.deploy = Deploy(path=os.path.dirname(os.path.abspath(__file__)), filename="config_with_alias_and_version.json")
